...
@Composable
fun BloodDonationPage(navController: NavController) {
    Scaffold(
        topBar = { TopApplicationBar("Kan Bağışı", navController) },
        content = { paddingValues -> BloodDonationMap(paddingValues) },
...
@Composable
fun BloodDonationMap(paddingValues: PaddingValues) {
    Column(modifier = fillmaxwid.fillMaxSize()) {
        ...
        val url = "https://www.google.com/maps/search/kan+elazığ+bağışı+noktası+" + location.value.text + "/data=!3m1!4b1!4m3!2m2!5m1!10e2?authuser=0&entry=ttu"
        AndroidView(factory = {
            WebView(it).apply {
